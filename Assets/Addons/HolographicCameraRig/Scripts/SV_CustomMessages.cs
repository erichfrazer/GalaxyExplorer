...
namespace SpectatorView
{
    public class SV_CustomMessages : SpectatorView.SV_Singleton<SV_CustomMessages>
    {
        ...
        {
#if UNITY_EDITOR
            if (this.serverConnection != null && this.serverConnection.IsConnected() && HolographicCameraManager.Instance != null && SharingSessionTracker.Instance != null)
            {
                if (SharingStage.Instance == null || SharingSessionTracker.Instance.UserIds.Count <= 1)
                {
                    return;
                }
